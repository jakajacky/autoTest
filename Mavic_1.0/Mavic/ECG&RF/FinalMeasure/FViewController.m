...
//                                       };
//            NSLog(@"i%ld-q%ld-i%ld-q%ld-i%ld-q%ld-i%ld-q%ld", [[[MeasureManager defaultManager].rb_i_str componentsSeparatedByString:@","] count],[[[MeasureManager defaultManager].rb_q_str componentsSeparatedByString:@","] count],[[[MeasureManager defaultManager].rt_i_str componentsSeparatedByString:@","] count],[[[MeasureManager defaultManager].rt_q_str componentsSeparatedByString:@","] count],[[[MeasureManager defaultManager].lb_i_str componentsSeparatedByString:@","] count],
//                  [[[MeasureManager defaultManager].lb_q_str componentsSeparatedByString:@","] count],[[[MeasureManager defaultManager].lt_i_str componentsSeparatedByString:@","] count],[[[MeasureManager defaultManager].lt_q_str componentsSeparatedByString:@","] count]);
////            if ([[[MeasureManager defaultManager].rb_i_str componentsSeparatedByString:@","] count] != [[[MeasureManager defaultManager].rb_q_str componentsSeparatedByString:@","] count]) {
////                
////            }
//            [self.api uploadResultWithResult:@[lb_item, rb_item, lt_item, rt_item] completion:^(BOOL success, id result, NSString *msg) {
//                
//                if (success) {
//                    [self.resultModel reloadDataWith:result];
...
//                    result.resultModel = self.resultModel;
//                    // 推出结果控制器
//                    [_mainVC.backBtn removeAllTargets];
//                    
//                    _mainVC    = (ViewController *)self.parentViewController.parentViewController;
...
-(void)writeToFileWithString:(NSData *)string withFileName:(NSString*)fileName{
    ...
    NSString *fileName1 = [path stringByAppendingPathComponent:fileName]; //@"125Hz.txt"
...
- (void)commit {
    ...
    NSString *dateS = [now stringWithFormat:@"HH_mm_ss" timeZone:[NSTimeZone systemTimeZone] locale:[NSLocale currentLocale]];
    for (NSData *d in [MeasureManager defaultManager].lt_logs) {
        [self writeToFileWithString:d withFileName:[NSString stringWithFormat:@"%@_1.txt", dateS]];
    }
    for (NSData *d in [MeasureManager defaultManager].lb_logs) {
        [self writeToFileWithString:d withFileName:[NSString stringWithFormat:@"%@_2.txt", dateS]];
    }
    for (NSData *d in [MeasureManager defaultManager].rt_logs) {
        [self writeToFileWithString:d withFileName:[NSString stringWithFormat:@"%@_3.txt", dateS]];
    }
    for (NSData *d in [MeasureManager defaultManager].rb_logs) {
        [self writeToFileWithString:d withFileName:[NSString stringWithFormat:@"%@_4.txt", dateS]];
    }
...
- (void)checkLockSignal {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self progressT:countProgress/25.0];
    });
...
- (ResultModel *)resultModel {
    if (!_resultModel) {
        _resultModel = [[ResultModel alloc] init];
    }
    return _resultModel;
}
